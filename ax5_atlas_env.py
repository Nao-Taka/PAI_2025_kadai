...
from ax5_motion_controller import set_pose, get_pose, init_pose, init_pose_reset
from ax5_make_motion import ozigi, wark
...
class AtlasEnv(gym.Env):
    def __init__(self, is_direct=True, render_mode=None, history_length=3):
        ...
        self.vec_dim = 7 * 3  # 2x(foot, sune, momo), body の各ベクトル（3次元）×7
        obs_dim = (self.history_length + 1) * self.vec_dim
        self.observation_space = spaces.Box(low=-1.0, high=1.0, shape=(obs_dim,), dtype=np.float32)
    
        # アクション空間（次の姿勢ベクトル1セットを出力）
        act_dim = self.vec_dim  # foot〜body のベクトル（3D × 7）
        ...
        # 姿勢履歴・目標姿勢の保存
        self.past_poses = [{0:glm.vec3(0, 0, 1),
                            1:glm.vec3(0, 0, -1), 2:glm.vec3(0, 0, -1), 
                            3:glm.vec3(0, 0, -1), 4:glm.vec3(0, 0, -1), 
                            5:glm.vec3(0, 0, -1), 6:glm.vec3(0, 0, -1)}
        ...
        for i in range(1000):
            self.target_pose.append({0:glm.vec3(0, 0, 1), 1:glm.vec3(0, 0, -1),
                                    ...
                                    4:glm.vec3(0, 0, -1), 5:glm.vec3(0, 0, -1),
                                    6:glm.vec3(0, 0, -1)})
            
    def set_pose(self, motions):
        '''
        [
        {0:glm.vec3(0, 0, 1), 1:glm.vec3(0, 0, 1), 2:glm.vec3(0, 0, 1),
        3:glm.vec3(0, 0, 1), 4:glm.vec3(0, 0, 1), 5:glm.vec3(0, 0, 1),6:glm.vec3(0, 0, 1)}
        ]
        の形式でリストを渡す
        '''
    ...
    def __action2motion(self, action)-> dict[int, glm.vec3]:
        #actionは7x3次元のnumpy配列
        ret = {}
        EPS = 1e-6

        vectors = action.reshape(7, 3)
    ...
    def __reward(self, currentpose, targetpose):
        ret = 0
        reward_weights = [1.5, 1.7, 0.8, 1, 1.7, 0.8, 1]
    ...
    def __terminate_episode(self, curentpose):
        ...
        #足が横向きすぎ
        foot_pena_L = curentpose[1] * .5 + curentpose[2] * .5 
        foot_pena_R = curentpose[4] * .5 + curentpose[5] * .5 
        foot_pena = max( abs(foot_pena_L.z) , abs(foot_pena_R.z))
        if foot_pena < 0.3:
            return True
    
        #体倒れすぎ
        body_pena = abs( curentpose[0].z )
        if body_pena < 0.5:
            return True
        return False 
    ...
    def _on_step(self):
        ...
        if any(dones):
            ...
            if (self.n_episode % self.render_interval) == 0:
                ...
                #検証用の環境構築
                env = AtlasEnv(is_direct=False, render_mode='rgb_array', history_length=5)
                obs, _ = env.reset()
                done = False
                total_reward = 0
...
if __name__=='__main__':
    for pose in ['wark']:
        ...
        env = AtlasEnv(is_direct=True, render_mode='rgb_array', history_length=5)
        # check_env(env)
        if pose == 'ozigi':
            env.set_pose(ozigi())
        elif pose == 'wark':
        ...
        custom_calback = CustomCallback(save_dirpath=log_path, 
                                        render_interval=500,model_save_interval=500)
        ...
        if pose == 'stand':       
            ...
            model.load('atlas_rl_model_standing_stand')
        elif pose == 'wark':
            model.load('atlas_rl_model_standing_wark')
        ...
        model.save(f'atlas_rl_model_standing_{pose}2')
